  ...
  $("#doctor-lookup").submit(function(event) {
    ...
    doctorPromise.then(function(response) {
      ...
      for (let i = 0; i < doctorInfo.data[0].practices.length; i++) {
        ...
        let website;
        if (doctorInfo.data[0].practices[i].website) {
          website = doctorInfo.data[0].practices[i].website;
        } else {
          website = "none listed";
        }
        let acceptsNewPts;
        ...
        $(".output").append(`<div class=\"card\"><h4><span class=\"label\">Name (first and last): </span>${doctorInfo.data[0].practices[i].name}</h4><p><span class=\"label\">Phone number: </span>5032161150</p><p><span class=\"label\">Address:</span>${address}</p><p><span class=\"label\">Website: </span>${website}</p><p><span class=\"label\">Accepting New Patients: </span>${acceptsNewPts}</p></div>`);
      }
