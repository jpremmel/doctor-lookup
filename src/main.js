    ...
    doctorPromise.then(function(response) {
      ...
      for (let i = 0; i < doctorInfo.data.length; i++) {
          ...
          for (let j = 0; j < doctorInfo.data[i].practices.length; j++) {
            ...
            let practiceAddress = `<br>${doctorInfo.data[i].practices[j].visit_address.street}<br>${doctorInfo.data[i].practices[j].visit_address.street2}<br>${doctorInfo.data[i].practices[j].visit_address.city}, ${doctorInfo.data[i].practices[j].visit_address.state} ${doctorInfo.data[i].practices[j].visit_address.zip}`;
          ...
          $(".output").append(`<div class="card"><h4><span class="label">Name: </span>${name}</h4><p><span class="label">Practice(s): </span>${practiceNames}</p></div>`);
      }
